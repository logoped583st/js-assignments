...
function getRectangleString(width, height) {
    let topEdge = '', bottomEdge = '', inside = '';

    topEdge = '┌' + '─'.repeat(width - 2) + '┐\n';
    bottomEdge = '└' + '─'.repeat(width - 2) + '┘\n';
    inside = '│' + ' '.repeat(width - 2) + '│\n';

    return topEdge + inside.repeat(height - 2) + bottomEdge;
}
...
function encodeToRot13(str) {
    return str.replace(/[A-Za-z]/g, function (symbol) {
        return String.fromCharCode(symbol.charCodeAt(0) + (symbol.toUpperCase() <= "M" ? 13 : -13));
    });
}
